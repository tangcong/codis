...
func (p *FlushEncoder) EncodeReq(req *WriteRequest) error {
	if err := p.Conn.EncodeReq(req, false); err != nil {
		return err
	} else {
		p.nbuffered++
		return nil
...
func (p *FlushEncoder) EncodeRes(res *WriteResponse) error {
	if err := p.Conn.EncodeRes(res, false); err != nil {
		return err
	} else {
		p.nbuffered++
		return nil
